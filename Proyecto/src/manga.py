...
def best_scoring_by_more_than_n_people_manga(mangas,n=500):
    scoring = [(r.title, r.score, r.scored_by) for r in mangas if r.scored_by>=n]
    scoring.sort(key=lambda x:x[1], reverse=True)
    return scoring

def obtain_the_n_most_favourited_manga_by_status(mangas, status="Finished", n=3):
    favourited = [(r.title, r.favourites) for r in mangas if r.status==status]
    favourited.sort(key=lambda x:x[1], reverse=True)
    return favourited[:n]
